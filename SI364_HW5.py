...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/msettonHW5"
...
class User(db.Model):
    ...
    twitter_email = db.Column(db.String(64), unique=True)
    def __repr__(self):
...
class TweetForm(FlaskForm):
    ...
    email = StringField("What is your Twitter user email?",validators=[Required()])
...
def get_or_create_user(db_session, username, email):
    ...
    if user:
        ...
        user = User(twitter_username=username, twitter_email=email)
        db_session.add(user)
        db_session.commit()
        return user
